'''
    ...
    list() 创建一个空列表
    add(item)  将新项添加到列表，没有返回值，假设元素不存在列表中
    remove(item)  从列表中删除元素，需要一个参数，会修改列表，假设在列表中
    search(item)  搜索列表中的元素，需要一个参数，返回一个布尔值
    isEmpty()   判断是否为空，不需要参数，返回一个布尔值
    size()  返回列表的元素数。不需要参数，返回一个整数
    append(item)  在列表末端添加一个新元素。它需要一个参数，没有返回值
    index(item)   返回元素在列表中的位置，需要一个参数，返回位置索引值
    insert(pos,item)  在指定的位置插入一个新的新元素，需要两个参数，没有返回值
    pop() 从列表末端移除一个元素并返回它，不需要参数，假设列表中至少有一个元素
    pop(pos)    从指定的位置移除一个元素并返回他，需要一个位置参数，返回一个元素
